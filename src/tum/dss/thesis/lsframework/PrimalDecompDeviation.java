...
/**
 * Modified primal LP of the convex decomposition which minimizes the absolute deviation as Gurobi Model.
 * @author Florian Stallmann
 *
 */
public class PrimalDecompDeviation implements PrimalDecompositionInterface {
	private static final Logger log = Logger.getLogger( PrimalDecompDeviation.class.getName() );
	 ...
	 */
	public PrimalDecompDeviation(final EllipsoidMethodData data) {
		this.ell_data = data;
	}
...
//			obj = model.get(GRB.DoubleAttr.ObjVal);
	...
	public void createLP() {
		try {
			...
			// Set objective: minimize sum lambda_l * deviation
			double expected_welfare = max_welfare / integrality_gap;
			GRBLinExpr expr = new GRBLinExpr();
			for (int i = 0; i < ell_data.getNumIntegerSolutions(); i++) {
				expr.addTerm(FastMath.abs(ell_data.getIntegerSolution(i).getWelfare()-expected_welfare), x[i]);
			}
